...
| `stopped_early` | string | when present | Additive: the loop ended before `count` on purpose because the server rejected the test credentials (repeating would lock the account). Absent otherwise. |
